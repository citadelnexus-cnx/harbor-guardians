...
export const TERMINAL_STATES = ["RESOLVED", "EXPIRED", "DECLINED"];
function assertUniqueIds(label, ids) {
    const seen = new Set();
    for (const id of ids) {
        if (id === "")
            throw new EventLifecycleInvariantError(`${label} contains an empty id`);
        if (seen.has(id))
            throw new EventLifecycleInvariantError(`${label} contains duplicate id "${id}"`);
        seen.add(id);
    }
}
...
export function assertEventValid(event) {
    if (event.event_id === "")
        throw new EventLifecycleInvariantError("event_id must be non-empty");
 ...
 */
export function evaluateCondition(condition, observed) {
    switch (condition.kind) {
        case "harbor_resource_at_least": {
            const band = observed.harbor.resources[condition.resource];
            const value = condition.band === "safe" ? band.safe : condition.band === "exposed" ? band.exposed : band.safe + band.exposed;
        ...
        default: {
            ...
            throw new EventLifecycleInvariantError(`condition kind "${impossible.kind}" references state that does not exist at A3 (EVT4)`);
        }
    }
}
/** ALL triggers must hold for eligibility (Doc 15 §3). */
export function evaluateTriggers(event, observed) {
...
export function createEventInstance(event, instanceId) {
    assertEventValid(event);
    if (instanceId === "")
        throw new EventLifecycleInvariantError("instance_id must be non-empty");
    return {
 ...
 */
function descriptorFingerprint(effect) {
    const record = effect;
...
function descriptorSequence(effects) {
...
export function assertEventInstanceValid(event, instance) {
    assertEventValid(event);
    if (instance.event_id !== event.event_id) {
        throw new EventLifecycleInvariantError(`instance "${instance.instance_id}" belongs to event "${instance.event_id}", not "${event.event_id}"`);
    }
    ...
    if (resolvingOrLater) {
        ...
        if (!outcome) {
            throw new EventLifecycleInvariantError(`instance "${instance.instance_id}" selected unknown outcome "${instance.selected_outcome_id}"`);
        }
        ...
        if (staged !== declared) {
            throw new EventLifecycleInvariantError(`instance "${instance.instance_id}" staged effects [${staged}] differ from outcome "${outcome.outcome_id}" effects [${declared}] — staged descriptors must be preserved verbatim, ordered, with every field intact (EVT3)`);
        }
    }
    else {
        if (instance.selected_outcome_id !== null || instance.staged_effects.length > 0) {
            throw new EventLifecycleInvariantError(`instance "${instance.instance_id}" carries outcome/effect state before RESOLVING — staging happens exactly once at ACTIVE → RESOLVING`);
        }
    }
}
const fail = (instance, signal) => {
    throw new EventLifecycleInvariantError(`signal "${signal.kind}" is not a legal transition from ${instance.state} ` +
 ...
 */
export function advance(event, instance, signal) {
    assertEventInstanceValid(event, instance);
    const from = instance.state;
    if (TERMINAL_STATES.includes(from)) {
        throw new EventLifecycleInvariantError(`instance "${instance.instance_id}" is terminal (${from}) — no further transition is ever legal; ` +
    ...
    switch (from) {
        ...
        case "OFFERED": {
            ...
            if (signal.kind === "expire")
                return move("EXPIRED");
            return fail(instance, signal);
        }
        case "TRIGGERED": {
            if (signal.kind !== "begin")
                return fail(instance, signal);
            return move("ACTIVE");
        }
        case "ACTIVE": {
            ...
            if (!outcome) {
                throw new EventLifecycleInvariantError(`complete named unknown outcome "${signal.outcome_id}" for event "${event.event_id}"`);
            }
