 ...
 */
import { CORE_RESOURCES, deposit } from "./harbor-state.js";
...
export class RewardRouteNotImplementedError extends Error {
    route;
    constructor(route, lineId) {
        super(`reward route "${route}" (line ${lineId}) is FUTURE BUILD at Alpha A2 — ` +
...
const IMPLEMENTED_ROUTES = ["claim_ledger", "story_claim"];
/** Empty ledger state (world-creation identity — no packages, no story claims). */
export function createClaimLedgerState() {
    return { packages: [], story_claims: [] };
}
function assertPositiveFiniteAmount(label, value) {
...
export function totalRewardAmount(resources) {
    let total = 0;
    for (const [, amount] of resourceEntries(resources))
        total += amount;
    return total;
}
/** Empty contents envelope (flags/cosmetics/gear_refs schema-enforced empty at A2). */
function contentsOf(resources) {
...
function occupiedIds(ledger, pending) {
    const ids = new Set();
    for (const p of ledger.packages)
        ids.add(p.package_id);
    for (const s of ledger.story_claims)
        ids.add(s.claim_id);
    for (const r of pending)
        ids.add(r.pending_id);
    return ids;
}
/**
 ...
 */
export function slotOccupancy(ledger) {
    const per = {};
    for (const resource of CORE_RESOURCES) {
 ...
 */
function blockedSlots(ledger, resources, rules) {
 ...
 * duplication, L11).
 */
export function routeRewardPackage(ledger, pending, draft, rules) {
    ...
    if (ids.has(draft.package_id) || ids.has(storyClaimId)) {
        throw new ClaimLedgerInvariantError(`package_id "${draft.package_id}" already delivered (ledger, story, or pending) — a reward can never be duplicated (L11)`);
    }
 ...
 */
export function claimPackage(ledger, harbor, packageId, mode) {
    ...
    for (const [resource, held] of resourceEntries(pkg.held_remainder)) {
        ...
        if (transferred + remaining !== held) {
            throw new ClaimLedgerInvariantError(`partial-claim conservation violated for ${packageId}.${resource}: ${transferred} + ${remaining} != ${held} (L6)`);
        }
        if (transferred > 0)
            claimed[resource] = transferred;
        if (remaining > 0)
            newRemainder[resource] = remaining;
    }
 ...
 */
export function resolvePendingToLedger(ledger, pending, pendingId, rules) {
 ...
 */
export function assertClaimStateValid(ledger, pending) {
    const seen = new Set();
    const claim = (id, what) => {
        if (seen.has(id))
            throw new ClaimLedgerInvariantError(`duplicate id "${id}" (${what})`);
        seen.add(id);
    };
    for (const pkg of ledger.packages) {
        ...
        for (const [resource, amount] of held) {
            ...
            if (amount > original) {
                throw new ClaimLedgerInvariantError(`package "${pkg.package_id}" holds ${amount} ${resource} but was created with ${original} — value cannot grow in the Ledger (L1)`);
            }
        }
    }
