...
Milestone N+1 does not begin until milestone N is stable and **tested**. Milestone 0 is **closed** (see `M0_EXIT_EVIDENCE.md`); Alpha A0 is **closed** (EVT stubs registered fail-loud, registry 132); Alpha A1 is **closed** (harbor/resource spine, PR #15). Current scope: **Alpha A2 only** — Claim Ledger and Reward Routing (the minimal Claim Ledger / reward-routing spine: test-supplied reward packages represented, routed, partially claimed, blocked safely at capacity, and preserved without hidden loss; selected honest L-suite invariant conversion; save/load extension with migration only as needed), per an owner authorization dated 2026-07-17; the A2 boundary is recorded in `docs/alpha/ALPHA_A2_EXECUTION_BRIEF_v0.1.md`. **Alpha A3 is not authorized. No gameplay loop, no gameplay reward sources, no events, no expeditions, no combat, no raids, no guardians, no factions, no cargo voyages, no gameplay UI, no deployment, no production work.** Any scope beyond A2 requires a separate owner approval.

This scope section is amended only by a new owner authorization record (held privately outside this public repo, like the M0, A0, A1, and A2 records). If you believe scope has expanded, verify with the owner against the latest authorization record before acting — never assume.
