...
Milestone N+1 does not begin until milestone N is stable and **tested**. Milestone 0 is **closed** (see `M0_EXIT_EVIDENCE.md`); Alpha A0 is **closed** (EVT stubs registered fail-loud, registry 132); Alpha A1 is **closed** (harbor/resource spine, PR #15); Alpha A2 is **closed** (Claim Ledger / reward routing, PR #16). Current scope: **Alpha A3 Option A only** — Expedition and Event Skeleton, **lifecycle mechanics only** (typed event contracts and generated schema; test-fixture event definitions only — no real `/data` event content; the deterministic nine-label lifecycle state machine driven by test-harness-supplied signals; observable-state trigger evaluation limited to the implemented A1 harbor and A2 claim-ledger state; save migration only if required to prove EVT3 honestly; honest conversion of **EVT1–EVT4 only**), per an owner authorization dated 2026-07-18; the A3 boundary is recorded in `docs/alpha/ALPHA_A3_EXECUTION_BRIEF_v0.1.md`. **Not authorized: EVT5–EVT10 conversion, event reward generation, event-to-Claim-Ledger delivery, any new Claim Ledger `source_type` (the A2 `test_supplied` literal is unchanged), effect execution or dispatch, any gameplay loop, real event content, combat, raids, threat behavior, guardians, factions, cargo voyages, gear gameplay, Harbor Inbox/messages, gameplay UI, Tauri shell changes, deployment, production work, and Alpha A4 or later.** Any scope beyond A3 Option A requires a separate owner approval.
